...
<template>
  <nav class="fixed bg-secondary py-4 lg:py-3 xl:py-4 z-50 w-full">
    <div class="container flex justify-between items-center">
      <!-- Brand Logo -->
      <div class="brand-logo w-16 lg:w-20 2xl:w-24">
        <RouterLink to="/">
      ...
      <div class="hidden md:flex items-center md:gap-10 xl:gap-20 3xl:gap-28">
        <ul class="nav-links md:space-x-14 xl:space-x-24 3xl:space-x-38 md:flex lg:text-xl xl:text-2xl 3xl:text-3xl text-black font-bold">
          <template v-for="(navLink, i) in menuNavLinks" :key="i">
            <li
                class="cursor-pointer"
                :class="{ 'text-primary': currentSection === navLink.sectionId }"
